...
def calculate_transitive_dependants(dependencies):
    ...
    all_packages = list(dependencies.keys())  # Take a snapshot of all keys
    ...
    for package_version in all_packages:
        ...
        while queue:
            ...
            if current in visited:
                continue
            visited.add(current)
            transitive_dependants_count[current] += 1  # Count each node once per traversal
            for dependent in dependencies[current]:
                if dependent not in visited:
                    queue.append(dependent)

    # Remove self counts to get true transitive dependants
    for package in transitive_dependants_count:
        transitive_dependants_count[package] -= 1
